...
from AISpider.items.lockyer_valley_items import LockyerItem
import time
...
class LockyerValleySpider(scrapy.Spider):
    """
    lockyer valley 的爬虫，原网站用.net开发，页面的渲染都在服务器完成，因此需要动态爬取，类似的需求还有
    """
    name = "lockyer_valley"
    allowed_domains = ["lvrc-web.t1cloud.com"]
    start_urls = [
        "https://lvrc-web.t1cloud.com/T1PRDefault/WebApps/eProperty/P1/eTrack/eTrackApplicationSearchResults.aspx"]
    USER_TYPE = 'LV.WEBGUEST'
    ...
    start_date = '01/01/1991'  # 网站最早的记录为 04/03/1991
    ...
    def __init__(self, run_type='all', days=30, *args, **kwargs):
        ...
        """
        super(LockyerValleySpider, self).__init__(*args, **kwargs)
        self.run_type = run_type
        if days is not None:self.run_type = 'no_all'
        if days == None:
        # 如果没有传days默认为这个月的数据
    ...
    def get_detail_url(self, appid):
        base_url = 'https://lvrc-web.t1cloud.com/T1PRDefault/WebApps/eProperty/P1/eTrack/eTrackApplicationDetails.aspx'
        q_params = dict(r=self.USER_TYPE)
    ...
    def parse(self, response):
        ...
        search_class = 'all'
        if self.run_type == 'all':self.start_date = '01/01/1991'
        if True:
            all_month = get_all_month_(self.start_date,datetime.now().date().strftime('%d/%m/%Y'))
    ...
    def parse_detail(self, respond: HtmlResponse):
        ...
        try:     
            lodged_date = respond.meta.get('lodgement_date')
        ...
        if rows:
            ...
            temp_data1 = '' if rows[8].css('td::text').extract()[1].isspace() else \
                rows[8].css('td::text').extract()[1]
            try:     
                lodged_date = temp_data1
                time_array = time.strptime(lodged_date, '%d/%m/%Y')
                temp_data = int(time.mktime(time_array))
